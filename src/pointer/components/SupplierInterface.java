    ...
    private static JPanel getButtonPanel() {
        ...
        updateButton.addActionListener(e -> {
            ...
            if (row == -1 ) {
                ...
                String person = (String) table.getValueAt(row, 2);
                String phone = (String) table.getValueAt(row, 3);
                String address = (String) table.getValueAt(row, 4);
                try {
                    new SupplierUpdateDialog("更改供货商信息", true, id, name, person, phone,address).setVisible(true);
        ...
        insertButton.addActionListener(e -> {
            try {
                new SupplierInsertDialog("添加新供货商", true).setVisible(true);
            } catch (IOException ex) {
                throw new RuntimeException(ex);
            }
            updateTable();
